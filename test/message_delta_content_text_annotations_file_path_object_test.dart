import 'package:test/test.dart';
import 'package:openai_flutter_sdk/openai_flutter_sdk.dart';

// tests for MessageDeltaContentTextAnnotationsFilePathObject
void main() {
  final instance = MessageDeltaContentTextAnnotationsFilePathObjectBuilder();
  // TODO add properties to the builder and call build()

  group(MessageDeltaContentTextAnnotationsFilePathObject, () {
    // The index of the annotation in the text content part.
    // int index
    test('to test the property `index`', () async {
      // TODO
    });

    // Always `file_path`.
    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // The text in the message content that needs to be replaced.
    // String text
    test('to test the property `text`', () async {
      // TODO
    });

    // MessageDeltaContentTextAnnotationsFilePathObjectFilePath filePath
    test('to test the property `filePath`', () async {
      // TODO
    });

    // int startIndex
    test('to test the property `startIndex`', () async {
      // TODO
    });

    // int endIndex
    test('to test the property `endIndex`', () async {
      // TODO
    });

  });
}
